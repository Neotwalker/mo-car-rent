(() => {
  ...
  forms.forEach(form=>{
    ...
    const syncAppleDateStates=()=>{if(!isAppleDatePlatform)return;form.querySelectorAll('[data-car-date-control]').forEach(control=>{const input=control.querySelector('input[type=date]');if(!input)return;control.classList.add('is-apple-date');control.classList.toggle('is-empty',!input.value);});};
