(() => {
  ...
  const isAppleDatePlatform=(()=>{const ua=navigator.userAgent||'';const platform=navigator.platform||'';return /iPad|iPhone|iPod/.test(ua)||/Macintosh/.test(ua)||(platform==='MacIntel'&&navigator.maxTouchPoints>1);})();
  const PHONE_RULES={
  ...
  forms.forEach(form=>{
    ...
    const formatAppleDateLabel=(value)=>{if(!value)return 'дд.мм.гггг';const [y,m,d]=value.split('-').map(Number);return new Intl.DateTimeFormat('ru-RU',{day:'numeric',month:'short',year:'numeric'}).format(new Date(y,m-1,d));}; const syncAppleDateStates=()=>{if(!isAppleDatePlatform)return;form.querySelectorAll('[data-car-date-control]').forEach(control=>{const input=control.querySelector('input[type=date]');if(!input)return;control.classList.add('is-apple-date');control.classList.toggle('is-empty',!input.value);control.dataset.dateLabel=formatAppleDateLabel(input.value);});};
    const syncDates=()=>{if(!pickup||!ret)return; ret.min=pickup.value||today; if(ret.value && ret.value<ret.min)ret.value='';syncAppleDateStates();};
    ...
    pickup?.addEventListener('change',syncDates);ret?.addEventListener('change',syncAppleDateStates);pickup?.addEventListener('input',syncAppleDateStates);ret?.addEventListener('input',syncAppleDateStates);method?.addEventListener('change',syncDelivery);
    ...
    const hideSuccess=()=>{if(!success)return;success.classList.remove('is-visible');setTimeout(()=>{success.hidden=true;form.reset();showStep(0);syncDates();syncDelivery();syncPhone();syncAppleDateStates();},180)};
    ...
    syncDates();syncDelivery();syncPhone();syncAppleDateStates();showStep(0);
  });
})();